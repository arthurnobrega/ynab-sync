...
describe('YNAB flow', () => {
  it('should return action object for nubank credit card flow', async () => {
  ...
  it('should return action object for nubank account flow', async () => {
    const { transactions, ...remainingProps } = await executeNubankFlow({ flowType: FLOWTYPES[1] })
    const action = await executeYnabFlow(remainingProps, transactions)
  ...
  it('should return action object for bb flow', async () => {
    const { transactions, ...remainingProps } = await executeBBFlow({ flowType: FLOWTYPES[2] })
    const action = await executeYnabFlow(remainingProps, transactions)
  ...
  it('should return action object for bb flow without questions', async () => {
    const spy = jest.spyOn(inquirer, 'prompt')

    const args = { yesToAllOnce: true, password: '12456' }
    const { transactions, ...remainingProps } = await executeBBFlow({ args, ...savedActions[2] })
    const action = await executeYnabFlow({ ...remainingProps, args }, transactions)
